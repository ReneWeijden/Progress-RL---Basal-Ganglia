...
class neuralNetwork(nn.Module):
    def __init__(self, amount_of_inputs, amount_of_actions, dueling_q_learning):
        self.dueling_q_learning = dueling_q_learning
        super().__init__()
        if self.dueling_q_learning == True:
...
class lunarLanderAgent():
    ...
    def select_action(self, state, policy_network, dueling_q_learning):
...
class replayMemory():
    ...
    def learnFromMemory(self, device, policy_network, target_network, batch_size, optimizer, gamma, double_q_learning, deep_q_learning, dueling_q_learning):
        ...
        if dueling_q_learning:
            action_tensor = torch.tensor(action_array, dtype=torch.int64).to(device)
...
def plot(x_axis, y_axis):
    ...
    plt.clf()
    #plt.title('Training...')
    plt.xlabel('Amount of times target network updated from policy network')
...
#Determine the sort of Q-learning to be applied to the network
...
policy_network          = neuralNetwork(environmentManager.amount_of_state_inputs(), environmentManager.amount_of_actions(), dueling_q_learning).to(device)
target_network          = neuralNetwork(environmentManager.amount_of_state_inputs(), environmentManager.amount_of_actions(), dueling_q_learning).to(device)
...
for episode in range(AMOUNT_OF_EPISODES):
    ...
    while not environmentManager.done:
        action                        = agent.select_action(state, policy_network, dueling_q_learning) #select an action according to highest q-value retrieved from the policy network
        ...
        if replayMemory.canMemoryProvideSampleOfBatchSize(BATCH_SIZE):
            replayMemory.learnFromMemory(device, policy_network, target_network, BATCH_SIZE, optimizer, GAMMA, double_q_learning, deep_q_learning, dueling_q_learning)
    ...
    if target_network_update_needed == 0:
        ...
        target_network.eval()
    #amount_of_optimization_steps /= 100000
